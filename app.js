...
// --- API Configuration ---
const API_BASE_URL = localStorage.getItem('api_base_url') || 'http://localhost:3001/api';

// Utility function to fetch from backend
...
const MemoryManager = {
  get(key) {
    ...
    try {
      const item = localStorage.getItem(`whylayer_${key}`);
      if (item) return JSON.parse(item);
    } catch (e) { console.warn("LS Read Error", e); }

    // Fallback to Cookies
    const match = document.cookie.match(new RegExp('(^| )whylayer_' + key + '=([^;]+)'));
    if (match) {
  ...
  set(key, value) {
    ...
    // 1. LocalStorage
    try { localStorage.setItem(`whylayer_${key}`, valStr); } catch (e) { }

    // 2. Cookie (SameSite=Strict, Secure)
    const d = new Date();
    d.setTime(d.getTime() + (365 * 24 * 60 * 60 * 1000)); // 1 year
    let expires = "expires=" + d.toUTCString();
    document.cookie = `whylayer_${key}=${encodeURIComponent(valStr)};${expires};path=/;SameSite=Strict;Secure`;
  },

  clear() {
    // Clear all whylayer_ items
    Object.keys(localStorage).forEach(k => {
      if (k.startsWith('whylayer_')) localStorage.removeItem(k);
    });

    // Clear cookies
    document.cookie.split(";").forEach((c) => {
      const name = c.trim().split("=")[0];
      if (name.startsWith('whylayer_')) {
  ...
  isEnabled() {
    try {
      const val = localStorage.getItem('whylayer_memory_mode');
      // Check cookie as well if LS is missing
      if (!val) {
        const match = document.cookie.match(new RegExp('(^| )whylayer_memory_mode=([^;]+)'));
             ...
             <div class="flex items-start justify-between mb-6">
                 <div>
                     ...
                     <p class="text-sm text-zinc-400 mt-2 max-w-lg leading-relaxed">
                        When enabled, WhyLayer will use strictly-scoped cookies to remember your last active tab, selected service, and timeline position. 
...
let window_fakeExplanation = null;
let window_fakeSystemLogs = null;
...
async function loadDataFromBackend() {
  ...
  if (health) {
    console.log('✅ Loaded health data from backend');
    window_fakeExplanation = {
  ...
  if (logs) {
    console.log('✅ Loaded logs from backend');
    window_fakeSystemLogs = logs;
  }
  
  // If backend fails, use fallback data
  if (!window_fakeExplanation) {
    window_fakeExplanation = window.fakeExplanation || {
      question: "System Log Analysis Report",
      stats: {
  ...
  if (!window_fakeSystemLogs) {
    window_fakeSystemLogs = window.fakeSystemLogs || {};
  }
}
...
if (verifiedService) {
  ...
  // Load data from backend on initial page load
    ...
    <div class="animate-fade-in max-w-7xl mx-auto pb-12">
      ...
      <div class="mb-8 p-6 rounded-lg border border-indigo-500/30 bg-indigo-500/5 relative overflow-hidden group">
         ...
         <h3 class="flex items-center text-lg font-bold text-white mb-4 relative z-10">
            <i class="fas fa-sparkles text-indigo-400 mr-2"></i> Ask WhyLayer
         </h3>
  ...
  setTimeout(() => {
    try {
      ...
      if (result) {
        ...
        alert("This system does not expose logs publicly. Please connect via WhyLayer Agent or Webhook.");
      }
    } catch (e) {
